// ===== Beat Catcher — p5.js (final with minimal fixes) =====
...
function setup() {
  ...
  // looser threshold so onsets don't feel starved
  peak = new p5.PeakDetect(20, 150, 0.25, 18);
...
function updateAndDrawGame(dt) {
  ...
  if (song && song.isPlaying()) {
    ...
    // quicker decay to avoid long-term flattening after big hits
    bassPeak = Math.max(1, Math.max(bassPeak * 0.92, sumBass));
    const norm = sumBass / bassPeak;
    bassSmooth = lerp(bassSmooth, norm, 0.24);

    const rise = bassSmooth - lastBass;
    // slightly looser gates so lights don't stall
    const strong = (bassSmooth > 0.35 && rise > 0.020) || (bassSmooth > 0.70 && rise > 0.012);
    ...
    // fallback: if lights have been quiet for 600ms but bass is alive, pulse one
    if (!strong && (nowMs - lastSpotlightMs) > 600 && bassSmooth > 0.30) {
      addSpotlight(random(60, width - 60));
      lastSpotlightMs = nowMs;
    }
...
function startGame() {
  ...
  if (song) {
    song.stop();
    song.play();
    song.setLoop(false);         // change to true if you want endless loop
    fft.setInput(null);          // ensure clean routing
    fft.setInput(song);          // analyze only gameplay track
    song.onended(() => {
      if (gameState === PLAY) goGameOver();
    });
  } else {
    fft.setInput(null);
  }
